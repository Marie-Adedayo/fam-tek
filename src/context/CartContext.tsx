...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const dec = (id: string) =>
    setItems(prev => prev.map(ci => (ci.product.id === id ? { ...ci, qty: Math.max(1, ci.qty - 1) } : ci)))

  const clear = () => setItems([])

  const total = useMemo(
    () => items.reduce((sum, ci) => sum + ci.product.price * ci.qty, 0),
    [items]
  )

  return (
    <CartCtx.Provider value={{ items, add, remove, inc, dec, clear, total }}>
      {children}
    </CartCtx.Provider>
  )
}
